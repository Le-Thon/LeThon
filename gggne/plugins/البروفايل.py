...
@gggne.ar_cmd(pattern="تغيير بايو ([\s\S]*)")
async def _(event):
...
@gggne.ar_cmd(pattern="تغيير اسم ([\s\S]*)")
async def _(event):
...
@gggne.ar_cmd(pattern="تغيير صورة$")
async def _(event):
    reply_message = await event.get_reply_message()
    gggneevent = await edit_or_reply(event, "يتم تحميل الصورة لحسابي ...")
    ...
    try:
        ...
        await gggneevent.edit(str(e))
    else:
        if photo:
            await gggneevent.edit("**⌔∮ الان يتم التحميل الى تيليكرام ...**")
            if photo.endswith((".mp4", ".MP4")):
                size = os.stat(photo).st_size
                if size > 2097152:
                    await gggneevent.edit("⌔∮ الحجم يجب ان يكون اقل من 2 ميغا")
                    os.remove(photo)
                    return
                gggnepic = None
                gggnevideo = await event.client.upload_file(photo)
            else:
                gggnepic = await event.client.upload_file(photo)
                gggnevideo = None
            try:
                await event.client(
                    functions.photos.UploadProfilePhotoRequest(
                        file=gggnepic, video=gggnevideo, video_start_ts=0.01
                    )
                )
            except Exception as e:
                await gggneevent.edit(f"**خطأ:**\n{e}")
            else:
                await edit_or_reply(gggneevent, "**❃ تم بنجاح تغيير صورة الحساب**")
    try:
        os.remove(photo)
    except Exception as e:
        LOGS.info(str(e))
...
@gggne.ar_cmd(pattern="تغيير معرف ([\s\S]*)")
...
async def count(event):
    ...
    b = 0
    result = ""
    gggneevent = await edit_or_reply(event, "**⌔∮ يتم جمع معلومات حسابك**")
    ...
    await gggneevent.edit(result)


@gggne.ar_cmd(pattern="ازاله الصورة ?([\s\S]*)")
