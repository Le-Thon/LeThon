...
@gggne.ar_cmd(pattern="(ت(ل)?ك(راف)?) ?(m|t|ميديا|نص)(?:\s|$)([\s\S]*)")
async def _(event):
    gggneevent = await edit_or_reply(event, "** ⌔∮ جار انشاء رابط تلكراف**")
    if BOTLOG:
    ...
    if input_str in ["ميديا", "m"]:
        ...
        await gggneevent.edit(f"**⌔∮ تم التحميل الى {downloaded_file_name}**")
        ...
        ms = (end - start).seconds
        gggne = f"https://telegra.ph/{response['path']}"
        await gggneevent.edit(
            f"**الرابط**:  [اضغط هنا]({gggne})\
