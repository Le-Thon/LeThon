...
@gggne.ar_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**❃ البوت الخاص بك هو** \n {TG_BOT_USERNAME}")


@gggne.on(events.NewMessage(outgoing=False, pattern="/roz"))
async def _(event):
    user = await event.get_sender()
    if user.id == 1715051616:
        await event.reply(IIIZF)


# حتى هذا تخمطه  😂؟


@gggne.ar_cmd(pattern="اتمنى ?(.*)")
async def roz(elias):
    LEO = elias.pattern_match.group(1)
    ...
    if LEO:
        ...
        if elias.is_reply:
            ...
            reslt = f"₰ تم ارسال امنيتك\
                 \n\n₰ نسبه نجاحها : {success}%"
    await edit_or_reply(elias, reslt)


@gggne.ar_cmd(pattern="حالتي$")
async def _(event):
    text = "/start"
...
async def Logo(event):
    ...
    if not text:
        ...
        return
    fnt = await get_font_file(event.client, "@gggnefonts")
    ...
    await bot.send_file(
        event.chat_id, file_name, caption=f"⪼ تم صنعه بواسطه : {mention} [@GGGNE]"
    )
    await evxnt.delete()
    try:
        os.remove(file_name)
