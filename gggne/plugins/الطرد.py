...
async def ban_user(chat_id, i, rights):
    try:
        await gggne(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)


@gggne.ar_cmd(pattern="حذف المحظورين(?: |$)(.*)")
async def _(event):
...
@gggne.ar_cmd(pattern=r"تفليش$")
async def _(event):
    await event.delete()
    messagelocation = event.to_id
    async for user in event.iiizf.iter_participants(messagelocation):
        user_id = user.id
        try:
            await event.iiizf.edit_permissions(messagelocation, user_id, view_messages=False)
        except:
            pass


@gggne.ar_cmd(pattern="المحذوفين ?([\s\S]*)")
...
@gggne.ar_cmd(pattern="احصائيات الاعضاء ?([\s\S]*)")
async def _(event):
