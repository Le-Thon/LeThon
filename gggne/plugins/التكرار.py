...
async def spam_function(event, RR7PP, gggne, sleeptimem, sleeptimet, DelaySpam=False):

    counter = int(gggne[0])
    if len(gggne) == 2:
        spam_message = str(gggne[1])
        ...
        for _ in range(counter):
            ...
            await _gggneutils.unsavegif(event, RR7PP)
...
async def spammer(event):
    ...
    await spam_function(event, RR7PP, gggne, sleeptimem, sleeptimet)


@gggne.ar_cmd(pattern=f"{TKRAR} (.*)")
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, gggne, sleeptimem, sleeptimet, DelaySpam=True)


@gggne.ar_cmd(pattern="تكرار الملصق$")
...
@gggne.ar_cmd(pattern="ايقاف التكرار ?(.*)")
