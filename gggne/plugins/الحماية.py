...
from gggne.core.logger import logging
...
# ترجمه وكتابة فريق ليثون
@gggne.ar_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@gggne.ar_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
# ترجمه وكتابة فريق ليثون
@gggne.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
# ترجمه وكتابة فريق ليثون
@gggne.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@gggne.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "- - عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك "
        return await event.answer(text, cache_time=0, alert=True)
...
# ترجمه وكتابة فريق ليثون
@gggne.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "- - عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك "
        return await event.answer(text, cache_time=0, alert=True)
...
@gggne.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = " عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك "
        return await event.answer(text, cache_time=0, alert=True)
    ...
    try:
        ...
        PM_WARNS = {}  # ترجمه وكتابة فريق ليثون
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
@gggne.ar_cmd(pattern="الحماية (تشغيل|تعطيل)$")  # ترجمه وكتابة فريق ليثون
...
async def pmpermit_on(event):
    input_str = event.pattern_match.group(1)
    if input_str == "تعطيل":
        if gvarstatus("pmmenu") is None:  # ترجمه وكتابة فريق ليثون
...
@gggne.ar_cmd(pattern="(س|سماح)(?:\s|$)([\s\S]*)")
...
@gggne.ar_cmd(pattern="(ر|رفض)(?:\s|$)([\s\S]*)")
...
@gggne.ar_cmd(pattern="بلوك(?:\s|$)([\s\S]*)")
...
@gggne.ar_cmd(pattern="الخاص بلوك(?:\s|$)([\s\S]*)")
async def block_p_m(event):
    if gvarstatus("pmpermit") is None:
...
@gggne.ar_cmd(pattern="انبلوك(?:\s|$)([\s\S]*)")
...
# ترجمه وكتابة فريق ليثون
@gggne.ar_cmd(pattern="المسموح لهم$")
async def approve_p_m(event):
    if gvarstatus("pmpermit") is None:
        return await edit_delete(
            event,
    ...
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائمة الحماية ليثون.txt",
