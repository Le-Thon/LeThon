...
from gggne import HEROKU_APP, UPSTREAM_REPO_URL, gggne
...
async def update_bot(event, repo, ups_rem, ac_br):
    ...
    jasem = await event.edit(
        "**- تم التحديث بنجاح**\n" "جـار اعادة تشغيل البوت يرجى الانتظار"
    )
    await event.client.reload(jasem)
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    if HEROKU_APP_NAME is None:
        await event.edit(
            "**- يجب عليك وضع فار اسم التطبيق الخاص بك للتحديث**"
        )
        repo.__del__()
        return
    heroku_app = next(
    ...
    try:
        ...
        LOGS.error(e)
    try:
        add_to_collectionlist("restart_update", [jasem.chat_id, jasem.id])
    except Exception as e:
        LOGS.error(e)
    ups_rem.fetch(ac_br)
    repo.git.reset("--hard", "FETCH_HEAD")
...
@gggne.ar_cmd(pattern="تحديث(| الان)?$")
...
@gggne.ar_cmd(pattern="تحديث التنصيب$",)
async def upstream(event):
    if ENV:
