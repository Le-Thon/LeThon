...
from ..helpers.utils import _gggnetools
...
def verify_cond(gggnearray, text):
    return any(i in text for i in gggnearray)


def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"gggne_{userid}_{pack}_anim"
    elif is_video:
        return f"gggne_{userid}_{pack}_vid"
    return f"gggne_{userid}_{pack}"


def char_is_emoji(character):
    return character in gggneemoji.UNICODE_EMOJI["en"]
...
async def delpack(gggneevent, conv, cmd, args, packname):
    try:
        await conv.send_message(cmd)
    except YouBlockedUserError:
        await gggneevent.edit("لقد قمت بحظر البوت @stickers . قم بألغاء الحظر عن البوت.")
        return None, None
...
async def newpacksticker(
    ...
    try:
        ...
        await gggneevent.edit("لقد قمت بحظر البوت @stickers قم بفك الحظر وحاول مره اخرى.")
        if not pkang:
...
async def add_to_pack(
    ...
    try:
        ...
        await gggneevent.edit("لقد قمت بحظر البوت @stickers bot. قم بفك الحظر عنه واعد المحاولة.")
        if not pkang:
            return None, None
        return None, None
    await conv.get_response()
    ...
    while ("50" in x.text) or ("120" in x.text):
        ...
        await gggneevent.edit(f"`تبديل الحزمه الى {pack} بسبب امتلائها`")
        await conv.send_message(packname)
        x = await conv.get_response()
...
async def kang(args):  # sourcery no-metrics
    ...
    if not user.username:
        try:
            ...
            username = f"gggne_{user.id}"
    else:
        username = user.username
    userid = user.id
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            gggneevent = await edit_or_reply(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
            ...
            if message.media.document.mime_type == "video/webm":
                ...
                for attribute in attributes:
                    if isinstance(attribute, DocumentAttributeSticker):
                        if message.media.document.size / 1024 > 255:
                            ...
                            sticker = await animator(message, args, gggneevent)
                            await edit_or_reply(
                                gggneevent, f"`{random.choice(KANGING_STR)}`"
                            )
                        else:
                            gggneevent = await edit_or_reply(
                ...
                gggneevent = await edit_or_reply(args, "__⌛ جارِ التحميل عزيزي..__")
                sticker = await animator(message, args, gggneevent)
                await edit_or_reply(gggneevent, f"`{random.choice(KANGING_STR)}`")
        else:
        ...
        if len(splat) == 2:
            if char_is_emoji(splat[0][0]):
                if char_is_emoji(splat[1][0]):
                    return await gggneevent.edit("check `.info stickers`")
...
@gggne.on(admin_cmd(pattern="حزمة"))
async def gggnepkg(_):
    ...
    await edit_or_reply(
        _, f"**- تم اخذ الحزمه بنجاح ✓ \nالحزمه  → [اضغط هنا](https://t.me/addstickers/{gggne_Jep.set.short_name})**")
...
async def get_pack_info(event):
    ...
    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        return await gggneevent.edit("`This is not a sticker. Reply to a sticker.`")
...
async def cb_sticker(event):
    ...
    gggneevent = await edit_or_reply(event, "`جارِ البحث عن الحزمه....`")
    scraper = cloudscraper.create_scraper()
    text = scraper.get(combot_stickers_url + split).text
    soup = bs(text, "lxml")
    results = soup.find_all("div", {"class": "sticker-pack__header"})
    if not results:
        return await edit_delete(gggneevent, "`لم يتم العثور على النتائج :(.`", 5)
