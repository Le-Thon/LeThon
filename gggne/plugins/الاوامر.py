...
@gggne.ar_cmd(pattern="م15")
async def hi(event):
    await event.reply(
        "اوامر جلب الذاتية وامر الاضافة**:\n\n ◂  `.ضيف`\n❃ اكتب الامر مع رابط مجموعه التي تريد سحب اعضائها وارسل الامر في مجموعتك لسحبهم الى مجموعتك\n\n ◂ `.جلب الصورة`  او  `.ذاتية`\n❃ بالرد على صورة او فيديو ذاتية التدمير لحفظها وارسالها في الرسائل المحفوظة بسرية تامة الامر لليثون حصريا!"
    )
    await event.delete()


@gggne.ar_cmd(pattern="م16")
async def hi(event):
    await event.reply(
