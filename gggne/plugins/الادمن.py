...
from gggne.Config import Config
...
async def promote(event):
    ...
    if not user:
        return
    gggneevent = await eor(event, "⌔∮ جارِ رفع المستخدم في الدردشة")
    try:
        await event.client(EditAdminRequest(event.chat_id, user.id, new_rights, rank))
    except BadRequestError:
        return await gggneevent.edit(NO_PERM)
...
async def demote(event):
    ...
    if not user:
        return
    gggneevent = await eor(event, "⌔∮ جاري تنزيل المستخدم من رتبة الاشراف")
    newrights = ChatAdminRights(
        add_admins=None,
        invite_users=None,
        change_info=None,
...
async def _ban_person(event):
    ...
    gggneevent = await eor(event, "**⌔∮ تم حظر هذا المستخدم بنجاح**")
    try:
...
async def nothanos(event):
    ...
    if not user:
        return
    gggneevent = await eor(event, "⌔∮ جار الغاء حظر المستخدم")
    try:
        ...
        await gggneevent.edit("⌔∮ لقد حدث خطأ اثناء محاولة الغاء الحظر")
    except Exception as e:
        await gggneevent.edit(f"**خطأ :**\n`{e}`")


@gggne.ar_cmd(incoming=True)
...
@gggne.ar_cmd(pattern="كتم(?:\s|$)([\s\S]*)")
...
@gggne.ar_cmd(pattern="الغاء كتم(?:\s|$)([\s\S]*)")
...
@gggne.ar_cmd(pattern="تثبيت( بالاشعار|$)")
async def pin(event):
...
@gggne.ar_cmd(pattern="الغاء تثبيت( الكل|$)")
async def pin(event):
