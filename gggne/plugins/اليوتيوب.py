...
from . import BOTLOG, BOTLOG_CHATID, gggne
...
@gggne.ar_cmd(pattern="تحميل صوتي(?:\s|$)([\s\S]*)")
async def download_audio(event):
    ...
    for url in urls:
        try:
            ...
            vid_data = {"title": url, "uploader": "gggne", "formats": []}
        startTime = time()
        ...
        for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
            if _path.lower().endswith((".jpg", ".png", ".webp")):
                ...
                _fpath = _path
        if not _fpath:
            return await edit_delete(gggneevent, "**❃ غير قادر على الرفع**")
...
@gggne.ar_cmd(pattern="تحميل فيديو(?:\s|$)([\s\S]*)")
async def download_video(event):
    ...
    reply_to_id = await reply_id(event)
    for url in urls:
        ytdl_data = await ytdl_down(gggneevent, video_opts, url)
        if ytdl_down is None:
            return
        try:
            f = pathlib.Path("gggne_ytv.mp4")
            print(f)
            gggnethumb = pathlib.Path("gggne_ytv.jpg")
            if not os.path.exists(gggnethumb):
                gggnethumb = pathlib.Path("gggne_ytv.webp")
            if not os.path.exists(gggnethumb):
                gggnethumb = None
            await gggneevent.edit(
                f"**⌔∮ جار رفع الفيديو :**\
            ...
            os.remove(f)
            if gggnethumb:
                os.remove(gggnethumb)
        except TypeError:
            await asyncio.sleep(2)
    await event.delete()


@gggne.ar_cmd(pattern="انستا(?: |$)([\s\S]*)")
async def insta_dl(event):
    ...
    v2 = "@videomaniacbot"
    media_list = []
    gggneevent = await edit_or_reply(event, "**⪼ جار التحميل انتظر قليلا**")
    async with event.client.conversation(v1) as conv:
        try:
            v1_flag = await conv.send_message("/start")
        except YouBlockedUserError:
            await edit_or_reply(
                gggneevent, "**⪼ لقد حدث خطأ يرجى الغاء الحظر و المحاولة مره ثانيه**"
            )
            await gggne(unblock("instasave_bot"))
            v1_flag = await conv.send_message("/start")
        ...
        await edit_or_reply(gggneevent, "**⪼ جار التحميل من مصدر اخر**")
        async with event.client.conversation(v2) as conv:
            try:
                ...
                await gggne(unblock("videomaniacbot"))
                v2_flag = await conv.send_message("/start")
...
@gggne.ar_cmd(pattern="نتائج(?: |$)(\d*)? ?([\s\S]*)")
