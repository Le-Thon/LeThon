...
async def get_users(show):
    ...
    gggneevent = await edit_or_reply(show, "**⌔∮ جاري سحب قائمة معرّفات الأعضاء 🝛**")
    try:
        ...
        mentions += " " + str(e) + "\n"
    await edit_or_reply(gggneevent, mentions)


@gggne.ar_cmd(pattern="معلومات(?: |$)(.*)")
async def info(event):
    gggneevent = await edit_or_reply(event, "**⪼ يتمّ جلب معلومات الدردشة، إنتظر ⅏**")
    chat = await get_chatinfo(event, gggneevent)
    caption = await fetch_info(chat, event)
    try:
        await gggneevent.edit(caption, parse_mode="html")
    except Exception as e:
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, f"**⌔∮ هناك خطأ في معلومات الدردشة ✕ : **\n`{str(e)}`"
            )
        await gggneevent.edit("**⌔∮ حدث خطأ ما، يرجى التحقق من الأمر ⎌**")


async def get_chatinfo(event, gggneevent):
    ...
    try:
        ...
        try:
            ...
            await gggneevent.edit("**⌔∮ لم يتمّ العثور على القناة/المجموعة ✕**")
            return None
            ...
            await gggneevent.edit("**⌔∮ القناة أو المجموعة الخارقة غير موجودة ✕**")
            return None
        except (TypeError, ValueError) as err:
            await gggneevent.edit(str(err))
            return None
    return chat_info
