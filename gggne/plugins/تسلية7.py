...
from ..helpers.utils import _gggnetools, reply_id
...
async def catbot(event):
    ...
    output = await _gggnetools.media_to_pic(event, replied)
    if output[1] is None:
    ...
    gggne = f"https://telegra.ph{response[0]}"
    gggne = await trash(gggne)
    os.remove(download_location)
    await output[0].delete()
    await event.client.send_file(event.chat_id, gggne, reply_to=rozid)
