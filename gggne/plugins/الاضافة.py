...
@gggne.ar_cmd(pattern="انضمام ([\s\S]*)")
async def lol(event):
    a = event.text
    bol = a[5:]
    ...
    try:
        await gggne(functions.channels.JoinChannelRequest(bol))
        await event.edit("**- تم الانضمام بنجاح  ✓**")
    except Exception as e:
        await event.edit(str(e))


@gggne.ar_cmd(pattern="اضافه ([\s\S]*)")
async def _(event):
...
@gggne.ar_cmd(pattern="ضيف ([\s\S]*)", groups_only=True)
async def get_users(event):
    legen_ = event.text[10:]
    gggne_chat = legen_.lower
    restricted = ["@GGGNE", "@QHR_1"]
    gggne = await edit_or_reply(event, f"**جارِ اضأفه الاعضاء من  ** {legen_}")
    if gggne_chat in restricted:
        return await gggne.edit(
            event, "**- لا يمكنك اخذ الاعضاء من مجموعه السورس العب بعيد ابني  :)**"
        )
    sender = await event.get_sender()
    me = await event.client.get_me()
    if not sender.id == me.id:
        await gggne.edit("**▾∮ تتم العملية انتظر قليلا ...**")
    else:
        await gggne.edit("**▾∮ تتم العملية انتظر قليلا ...**")
    if event.is_private:
        return await gggne.edit("- لا يمكنك اضافه الاعضاء هنا")
    s = 0
    f = 0
    error = "None"
    await gggne.edit(
    ...
    async for user in event.client.iter_participants(event.pattern_match.group(1)):
        try:
            ...
            tol = f"@{user.username}"
            lol = tol.split("`")
            await gggne(InviteToChannelRequest(channel=event.chat_id, users=lol))
            s = s + 1
            await gggne.edit(
                f"**▾∮تتم الأضافة **\n\n• اضيف `{s}` \n•  خطأ بأضافة `{f}` \n\n**× اخر خطأ:** `{error}`"
            )
...
@gggne.ar_cmd(pattern="تجميع الاعضاء$")
...
@gggne.ar_cmd(pattern="اضف الاعضاء$")
