...
@gggne.ar_cmd(incoming=True, groups_only=True)
async def on_new_message(event):
    name = event.raw_text
    snips = sqlrz.get_chat_blacklist(event.chat_id)
    gggneadmin = await is_admin(event.client, event.chat_id, event.client.uid)
    if not gggneadmin:
        return
    for snip in snips:
...
@gggne.ar_cmd(pattern="منع(?:\s|$)([\s\S]*)")
async def _(event):
...
@gggne.ar_cmd(pattern="الغاء منع(?:\s|$)([\s\S]*)")
async def _(event):
    text = event.pattern_match.group(1)
    to_unblacklist = list(
        {trigger.strip() for trigger in text.split("\n") if trigger.strip()}
...
@gggne.ar_cmd(pattern="قائمة المنع$")
async def _(event):
