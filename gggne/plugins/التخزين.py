...
@gggne.ar_cmd(incoming=True, func=lambda e: e.mentioned, edited=False, forword=None)
...
@gggne.ar_cmd(pattern="خزن(?:\s|$)([\s\S]*)")
...
@gggne.ar_cmd(pattern="تفعيل التخزين$")
...
@gggne.ar_cmd(pattern="تعطيل التخزين$")
async def set_no_log_p_m(event):
    if Config.PM_LOGGER_GROUP_ID != -100:
        chat = await event.get_chat()
...
@gggne.ar_cmd(pattern="تخزين الخاص (تشغيل|ايقاف)$")
...
@gggne.ar_cmd(pattern="تخزين الكروبات (تشغيل|ايقاف)$")
