...
async def admin_groups(gggne):
    gggnegroups = []
    async for dialog in gggne.iter_dialogs():
...
async def extract_time(gggne, time_val):
    if any(time_val.endswith(unit) for unit in ("s", "m", "h", "d", "w")):
        unit = time_val[-1]
        time_num = time_val[:-1]  # type: str
        if not time_num.isdigit():
            await gggne.edit("**- هذا الرقم غير صحيح  !**")
            return None
        if unit == "s":
