        ...
        **kwargs,
        ...
        def decorator(func):  # sourcery no-metrics
            ...
            if pattern is not None:
                ...
                if edited:
                    gggne.add_event_handler(func, events.MessageEdited(**kwargs))
                gggne.add_event_handler(func, events.NewMessage(**kwargs))
            return wrapper
    ...
    def bot_cmd(
        ...
        **kwargs,
        ...
        def decorator(func):
            async def wrapper(check):
                try:
                    ...
                    if not disable_errors:
                        ...
                        text = "**⪼ تقرير خطأ ليثون 𓆰،**\n\n"
                        link = "[هنا](https://t.me/gggne)"
                        text += "يمكنك التبليغ عن هذه المشكله"
                        text += f"- فقط قم بتوجيه الرساله هنا {link}.\n"
                        text += "لم يتم حفظ اي شي عدا المشكله وتاريخ حدوثها\n\n"
                        text += f"**تقرير الخطأ : ** [{new['error']}]({pastelink})"
            ...
            if edited is True:
                gggne.tgbot.add_event_handler(func, events.MessageEdited(**kwargs))
            else:
                gggne.tgbot.add_event_handler(func, events.NewMessage(**kwargs))
