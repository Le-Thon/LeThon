...
def main_menu():
    text = f"**▾∮ مرحبا عزيزي {mention}**\n**▾اليكَ قائمة بازرار مضمنة لاوامر ↫**⍣ⵧⵧⵧⵧⵧLeThonⵧⵧⵧⵧⵧ⍣**\n[LETHON USER BOT 🧸♥](https://t.me/gggne)\n\n"
    buttons = [
...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**gggne"):
            buttons = [
                (
                    Button.inline("Stats", data="stats"),
                    Button.url("DEV", "https://t.me/GGGNE"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./gggne", "troll.txt")
            try:
            ...
            try:
                ...
                try:
                    ...
                    if u.username:
                        ...
                        LEO = f"[{u.first_name}](tg://user?id={u.id})"
                    u = int(u.id)
                except ValueError:
                    LEO = f"[user](tg://user?id={u})"
            except ValueError:
                # if u is username
                try:
            ...
            result = builder.article(
                ...
                text=f"🔒 هذه همسة سرية الى {LEO}, هو فقط من يستطيع رؤيتها",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./gggne", "secrets.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./gggne", "hide.txt")
            try:
            ...
            result = builder.article(
                title="gggne Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب ليثون **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/GGGNE"),
                Button.url(
                    "المطور",
                    "t.me/GGGNE",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل ليثون من نصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[LEITHON USERBOT 🧸♥](https://t.me/GGGNE)",
...
@gggne.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@gggne.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@gggne.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@gggne.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
